...
static void *OnStockStart() {
    ...
    struct stockplugin* stock = (struct stockplugin*)calloc(1,
            sizeof(struct stockplugin));
    stock->id = "stock";
    stock->name = "stock";
    stock->version = "1.0.0";
    stock->provider = "yg";
...
static handler_t OnStockMessage(struct server *srv, int fd, void *data,
        int len) {
	stock_logic::StockLogic::GetInstance()->OnStockMessage(srv,
            fd, data, len);
    return HANDLER_GO_ON;
}
...
static handler_t OnBroadcastMessage(struct server* srv, int fd, void *data,
        int len) {
	stock_logic::StockLogic::GetInstance()->OnBroadcastMessage(srv,
            fd, data, len);
    return HANDLER_GO_ON;
}
...
static handler_t OnTimeOut(struct server* srv, char* id, int opcode, int time) {
	stock_logic::StockLogic::GetInstance()->OnTimeout(srv,
            id, opcode, time);
    return HANDLER_GO_ON;
}
