...
void StockUserManager::DeleteOldEventsData() {
	...
	std::map<std::string, BasicIndustryInfo>& basic_industry_infos = industry_info.industry_info_map_;
	std::map<std::string, BasicIndustryInfo>::iterator basic_industry_iter = basic_industry_infos.begin();
	for (; basic_industry_iter != basic_industry_infos.end(); basic_industry_iter++) {
		StockUtil::Instance()->delete_old_yield_info(basic_industry_iter->second.industry_yield_infos_);
	}
...
bool StockUserManager::GetYieldJsonByName(std::string& cycle_type, std::string& start_date,
		std::string& industry_name, std::string& yield_json) {
	INDUSTRYINFO_MAP& industry_info = stock_user_cache_->industry_info_;
	if (industry_info.check_event(industry_name)) {
		BasicIndustryInfo& basic_info = industry_info.get_industry_by_name(industry_name);
		if ("current" == cycle_type) {
			std::map<int, YieldInfoUnit>& hs300_yield_data = stock_user_cache_->get_hs300_yield_data();
			return basic_info.get_chart_json(yield_json, hs300_yield_data);
		}
		else {
			std::map<std::string, HistDataPerDay>& hs300_hist_data = stock_user_cache_->get_hs300_hist_data();
		    return basic_info.get_hist_data_json(hs300_hist_data, start_date, yield_json);
		}
	}
	else
		return false;
...
void StockUserManager::UpdateIndustryYieldInfo(int& current_trade_time) {
	...
	for (; iter != industry_map.end(); iter++) {
		...
		for (; it != basic_industry_info.stock_price_info_.end(); it++) {
			...
			double stock_yield = stock_basic_info.get_yield_by_time(current_trade_time);
			industry_changepoint += stock_yield * stock_market_value/basic_industry_info.industry_market_value();
		...
		if (0 != current_trade_time)
			basic_industry_info.add_industry_yield_info(current_trade_time, industry_changepoint);
...
void StockUserManager::UpdateYieldDataToDB() {
	...
	for (; stocks_iter != stocks_map.end(); stocks_iter++) {
		...
		std::map<int, YieldInfoUnit>& yield_map = total_info.basic_info_.get_yield_infos();
	...
	std::map<std::string, BasicIndustryInfo>& industry_info = stock_user_cache_->industry_info_.industry_info_map_;
	std::map<std::string, BasicIndustryInfo>::iterator industry_iter = industry_info.begin();
	for (; industry_iter != industry_info.end(); industry_iter++) {
		...
		std::map<int, YieldInfoUnit>& industry_yields = basic_info.industry_yield_infos_;
		std::map<int, YieldInfoUnit>::iterator industry_yield_iter = industry_yields.begin();
		for (; industry_yield_iter != industry_yields.end(); industry_yield_iter++) {
			stock_db_->UpdateYieldInfo(industry_name, industry_yield_iter->second.trade_time_,
					industry_yield_iter->second.yield_);
		}
	}
}

std::string StockUserManager::GetStockKLineByCode(std::string stock_code, std::string format, std::string& cycle_type,
	...
	if (stocks_map.end() != iter) {
		...
		if ("jsonp" == format)
			...
			std::map<int, YieldInfoUnit>& hs300_yield_data = stocks_map[HSSANBAI].basic_info_.yield_infos_;
			return iter->second.getCharmJson(hs300_yield_data);
		}
		else if ("bydate" == cycle_type) {
			std::map<std::string, HistDataPerDay>& hs300_hist_data = stock_user_cache_->get_hs300_hist_data();
			StockUtil::Instance()->get_json_of_hist_data(hs300_hist_data,
					start_date, total_info.hist_data_info_.stock_hist_data_, r_json);
			return r_json;
		}
		else
