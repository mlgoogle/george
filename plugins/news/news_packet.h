...
#ifndef _HOME_TIANYIHENG_TEST_JINDOWIN_RUNNER_JINDOWIN_PLUGINS_NEWS_NEWS_PACKET_H_
#define _HOME_TIANYIHENG_TEST_JINDOWIN_RUNNER_JINDOWIN_PLUGINS_NEWS_NEWS_PACKET_H_
...
class StockNewsRecv:public RecvLoginBase {
 public:
  ...
  inline std::string get_sec_name() const {return sec_name_;}
...
// 相关新闻阅读
class RelatedNewsRecv:public RecvLoginBase {
 public:
  explicit RelatedNewsRecv(NetBase *m)
    :RecvLoginBase(m) {
      Init();
  }
  void Init();
...
// 新闻相关
class NewsReferenceRecv:public RecvLoginBase {
 public:
  explicit NewsReferenceRecv(NetBase *m)
    :RecvLoginBase(m) {
     Init();
  }
  void Init();
...
class StockNewsSend:public SendPacketBase {
 public:
  ...
  void LimitMaxSize(int32 max_size);
...
// 新闻相关
class NewsReferencSend:public SendPacketBase {
 public:
  NewsReferencSend() {
    stock_.reset(new base_logic::ListValue());
    industry_.reset(new base_logic::ListValue());
    section_.reset(new base_logic::ListValue());
  }
  NetBase* release() {
    this->set_status(1);
    head_->Set("stock", stock_.release());
    head_->Set("industry", industry_.release());
    head_->Set("section", section_.release());
    return head_.release();
  }
  inline void AddStock(base_logic::Value* v) {
    stock_->Append(v);
  }
  inline void AddIndustry(base_logic::Value *v) {
    industry_->Append(v);
  }
  inline void AddSection(base_logic::Value *v) {
    section_->Append(v);
  }

 private:
  scoped_ptr<base_logic::ListValue> stock_;
  scoped_ptr<base_logic::ListValue> industry_;
  scoped_ptr<base_logic::ListValue> section_;
};

#endif  // _HOME_TIANYIHENG_TEST_JINDOWIN_RUNNER_JINDOWIN_PLUGINS_NEWS_NEWS_PACKET_H_
