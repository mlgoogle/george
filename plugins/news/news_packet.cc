...
void StockNewsRecv::Init() {
  ...
  do {
    ...
    std::string section;
    r = m_->GetString(L"section", &section);
    base::BasicUtil::UrlDecode(section, sec_name_);
...
void RelatedNewsRecv::Init() {
  m_->GetString(L"news_id", &news_id_);
  m_->GetString(L"news_date", &news_date_);
}

void NewsReferenceRecv::Init() {
  m_->GetString(L"news_id", &news_id_);
  m_->GetString(L"news_date", &news_date_);
}
//--------------------------------------- send ------------------------
...
void StockNewsSend::LimitMaxSize(int32 max_size) {
  if (max_size >= news_->GetSize()) {
    return;
  }
  while (news_->GetSize() > max_size) {
    news_->Remove(0, NULL);
  }
}
