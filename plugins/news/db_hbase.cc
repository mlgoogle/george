...
void DbHbase::GetRowData(std::string table_name, std::string row_key,
  ...
  try {
    ...
    LOG_DEBUG("Error,GetRowData");
    LOG_ERROR("Error,GetRowData");
  }
}
...
void DbHbase::GetColumnData(std::string table_name, std::string row_key,
  ...
  try {
    ...
    LOG_DEBUG("Error,GetColumnData");
    LOG_ERROR("Error,GetColumnData");
  }
}
...
void DbHbase::InitHbase(std::string host, int port, \
    std::string table, int num) {
  s_host_ = host;
  s_port_ = port;
  s_table_ = table;
  if (num <= 0) {
