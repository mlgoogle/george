...
void NewsInterface::OnNewsDetailsEvent(const int socket, DicValue* dic,
                        george_logic::PacketHead* packet) {
  int64 nid = 0;
  dic->GetBigInteger(L"nid",&nid);
  send::NewsList* news_list = new send::NewsList();
  news_memery_->QueryNewsDetails(nid, news_list);
  news_list->set_type(3);
  news_list->set_timestamp(time(NULL));
  SendPacket(socket, news_list, packet->attach_field(),
             NEWS_DETAILS_RLY, george_logic::NEWS_TYPE);
  if (news_list) {
    delete news_list;
    news_list = NULL;
  }
