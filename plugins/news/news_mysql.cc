...
int NewsMysql::QueryNewsDetails(int64 nid, News* news) {
  int err = 0;
  bool r = false;
  do {
    std::stringstream os;
    os << "call proc_QueryNewsDetails(" << nid << ")";
    ...
    r = mysql_engine_->ReadData(0, (base_logic::Value*)(dict.get()),
                                CallQueryNews);
    if (!r) {
      LOG_ERROR("QueryNewsDetails sql error!");
      err = SQL_EXEC_ERROR;
      break;
    }
    r = dict->GetList(L"resultvalue", &listvalue);
    if (!r || listvalue == NULL) {
      LOG_ERROR("QueryNewsDetails error!");
      break;
    }
...
int NewsMysql::QueryNews(int64 time, int64 counts, NewsMap* map) {
  ...
  do {
    ...
    r = mysql_engine_->ReadData(0, (base_logic::Value*)(dict.get()),
                                CallQueryNews);
    if (!r) {
      LOG_ERROR("QueryNews sql error");
      err = SQL_EXEC_ERROR;
      break;
    }
    r = dict->GetList(L"resultvalue", &listvalue);
    if (!r || listvalue == NULL) {
      LOG_ERROR("QueryNews error");
      break;
    }
    while (listvalue->GetSize()) {
      news::News news;
...
int NewsMysql::QueryStNews(int64 now_time, int64 old_time, SimpleMap* nmap) {
  ...
  do {
    ...
    r = mysql_engine_->ReadData(0, (base_logic::Value*)(dict.get()),
                                CallQueryStNews);
    if (!r) {
      LOG_ERROR("QueryStNews sql error!");
      err = SQL_EXEC_ERROR;
      break;
    }
    ...
    if (listvalue == NULL) {
      err = -1;
      LOG_ERROR("QueryStNews error listvalue is null");
      break;
    }
    while (listvalue->GetSize()) {
...
int NewsMysql::QueryRfNews(int64 id, int32 count, int32 type, std::string key,
  ...
  do {
    ...
    if (!r) {
      err = SQL_EXEC_ERROR;
      LOG_ERROR("QueryRfNews sql error!");
      break;
    }
    r = dict->GetList(L"resultvalue", &listvalue);
    if (!r || listvalue == NULL) {
        LOG_ERROR("QueryRfNews error!");
       break;
    }
    while (listvalue->GetSize()) {
      news::News news;
